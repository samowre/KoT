/*------------------------------------------------------------------------*/
/* Copyright (c) 2005 - 2008 Armin Biere, Johannes Kepler University.     */
/*------------------------------------------------------------------------*/
...
static void
my_print_clause (Clause * clause, FILE * file)
{
  assert (clause);
  fprintf (file, "%d ", clause->idx);
...
tracecheck_configuration (void)
{
  return
    ...
    "OS=\"" BOOLEFORCE_OS "\"\n"
    "ID=\"$Id: tracecheck.c,v 1.117 2008-09-03 08:29:23 biere Exp $\"\n"
    "CC=\"" BOOLEFORCE_CC "\"\n"
...
tracecheck_main (int argc, char **argv)
{
  ...
  for (i = 1; !done && !res && i < argc; i++)
      ...
      else if (!strcmp (argv[i], "--id"))
	{
	  /**INDENT-OFF**/
	  fprintf (output,
"$Id: tracecheck.c,v 1.117 2008-09-03 08:29:23 biere Exp $\n");
	  done = 1;
	  /**INDENT-ON**/
	}
